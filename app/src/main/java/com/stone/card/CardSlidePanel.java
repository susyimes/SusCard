...
public class CardSlidePanel extends ViewGroup {
    ...
    private int bottomMarginTop = 4;
    private int yOffsetStep = 4; // view叠加垂直偏移量的步长
    ...
    public static final int VANISH_TYPE_LEFT = 1;
    public static final int VANISH_TYPE_RIGHT = 0;
    ...
    private void animToSide(View changedView, float xvel, float yvel) {
        ...
        /*  if (yvel > Y_VEL_THRESHOLD || dy > Y_DISTANCE_THRESHOLD) {
            finalY = allWidth;
            finalX = dx * (childWith + initCenterViewY) / dy + initCenterViewX;
            flyType = VANISH_TYPE_RIGHT;
        } else if (yvel < -Y_VEL_THRESHOLD || dy < -Y_DISTANCE_THRESHOLD) {
            finalY = -childWith;
            finalX = dx * (childWith + initCenterViewY) / (-dy) + dx
                    + initCenterViewX;
            flyType = VANISH_TYPE_LEFT;
        }*/
